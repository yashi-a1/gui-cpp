    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const char* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (d.Overlaps(clip_rect))
                        (fg ? gui->foreground_drawlist() : window->DrawList)->AddRectFilled(d.Min, d.Max, draw->get_clr(clr->accent, 0.2f), var->style.frame_rounding);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_screen_rect.Overlaps(clip_rect))
                draw->line((fg ? gui->foreground_drawlist() : window->DrawList), rect.Min + ImVec2(animstate->offset, line_padding), ImVec2(rect.Min.x + animstate->offset, rect.Max.y - line_padding), draw->get_clr({ 1.f, 1.f, 1.f, 1.f }), SCALE(1));
...
widget_t c_widgets::text_field(std::string_view key)
{
    struct anim_t
    {
        bool active{ false };
    ...
    text_field_t* item_data = cfg->fill<text_field_t>(key.data());
    widget_t data = gui->register_item(item_data->name, ImVec2(gui->content_avail().x, SCALE(elements->textfield.height)));
    anim_t* anim = gui->anim_container<anim_t>(gui->get_window()->GetID((std::stringstream{} << data->id << "adj_field").str().data()));
    const ImRect icon{ ImVec2(data->rect.Max.x - SCALE(elements->textfield.padding * 2) - gui->text_size(font->get(icon_data, 12.f), "K").x, data->rect.GetCenter().y - SCALE(elements->textfield.size / 2)), ImVec2(data->rect.Max.x, data->rect.GetCenter().y + SCALE(elements->textfield.size / 2)) };
    const ImRect rect{ ImVec2(data->rect.Min.x, icon.Min.y), icon.GetBL() };

    gui->easing(anim->alpha, strlen(item_data->callback.data()) > 0 ? 1.f : 0.f, 6.f, static_easing);
    gui->easing(anim->offset, strlen(item_data->callback.data()) > 0 ? rect.GetWidth() / 2 : 0.f, 14.f, dynamic_easing);

    draw->rect_filled(gui->window_drawlist(), rect.Min, icon.Max, draw->get_clr(clr->widgets.hex_field), SCALE(elements->textfield.rounding));

    draw->text_clipped(gui->window_drawlist(), font->get(inter_medium_data, 14.f), rect.Min + ImVec2(anim->offset + SCALE(elements->textfield.padding), 0), rect.Max, draw->get_clr(clr->widgets.text_inactive, 1.f - anim->alpha), item_data->name.data(), gui->text_end(item_data->name.data()), NULL, ImVec2(0.f, 0.5f));
    draw->text_clipped(gui->window_drawlist(), font->get(icon_data, 12.f), icon.Min, icon.Max, draw->get_clr(clr->widgets.text_inactive), "K", NULL, NULL, ImVec2(0.5f, 0.5f));

    draw->separator();

    gui->push_font(font->get(inter_medium_data, 14.f));
    gui->push_var(style_var_frame_rounding, SCALE(elements->textfield.rounding));

    item_data->callback.resize(item_data->size);
    text_field_ex(item_data->name.data(), "", item_data->callback.data(), item_data->size, SCALE(elements->textfield.padding), SCALE(elements->textfield.line_padding), draw->get_clr(clr->widgets.text), false, rect, &anim->active, 0, 0, 0);
    item_data->callback.resize(strnlen(item_data->callback.data(), item_data->size));
...
widget_t c_widgets::hex_field(std::string_view name, char* buf, int size, float width)
{
    struct anim_t
    {
        bool active{ false };
    ...
    widget_t data = gui->register_item(name.data(), ImVec2(width, SCALE(elements->color_edit.button_size)));
    anim_t* anim = gui->anim_container<anim_t>(gui->get_window()->GetID((std::stringstream{} << data->id << "adj_field").str().data()));

    gui->easing(anim->alpha, strlen(buf) > 0 ? 1.f : 0.f, 6.f, static_easing);
    gui->easing(anim->offset, strlen(buf) > 0 ? data->rect.GetWidth() / 2 : 0.f, 14.f, dynamic_easing);

    data->value_changed = anim->active;

    draw->rect_filled(gui->window_drawlist(), data->rect.Min, data->rect.Max, draw->get_clr(clr->widgets.hex_field), SCALE(elements->textfield.rounding));

    draw->text_clipped(gui->window_drawlist(), font->get(inter_medium_data, 17.f), data->rect.Min + SCALE(elements->textfield.padding, 0), data->rect.Max, draw->get_clr(clr->widgets.text_inactive), "#", NULL, NULL, ImVec2(0.f, 0.5f));

    gui->push_font(font->get(inter_medium_data, 14.f));
    gui->push_var(style_var_frame_rounding, SCALE(elements->textfield.rounding));

    text_field_ex(name.data(), "", buf, size, SCALE(elements->textfield.padding * 2) + gui->text_size(font->get(inter_medium_data, 17.f), "#").x, SCALE(elements->textfield.line_padding), draw->get_clr(clr->widgets.text), false, data->rect, &anim->active, 0, 0, 0);
...
widget_t c_widgets::search_field(std::string_view name, char* buf, int size)
{
    struct text_field_state
    {
        bool active{ false };
    ...
    text_field_state* anim = gui->anim_container<text_field_state>(gui->get_window()->GetID((std::stringstream{} << gui->get_window()->GetID(name.data()) << "adj_field").str().data()));
    widget_t data = gui->register_item(name, SCALE(elements->search.size_opened_off) + ImVec2(anim->size, 0));

    gui->easing(anim->alpha, anim->active ? 1.f : 0.f, 6.f, static_easing);
    gui->easing(anim->offset, strlen(buf) > 0 ? data->rect.GetWidth() / 2 : 0.f, 14.f, dynamic_easing);
    gui->easing(anim->size, anim->active ? SCALE(elements->search.size_opened_no) : 0.f, 16.f, dynamic_easing);

    draw->rect_filled(gui->window_drawlist(), data->rect.Min, data->rect.Max, draw->get_clr(clr->window.child), SCALE(elements->search.rounding));
    draw->rect(gui->window_drawlist(), data->rect.Min, data->rect.Max, draw->get_clr(clr->window.rect), SCALE(elements->search.rounding));

    draw->text_clipped(gui->window_drawlist(), font->get(icon_data, 16), data->rect.Min, data->rect.Max - SCALE(12, 0), draw->get_clr(clr->widgets.text_inactive), "J", NULL, NULL, ImVec2(1.f, 0.5f));

    gui->push_font(font->get(inter_medium_data, 14));
    gui->push_var(style_var_frame_rounding, SCALE(elements->search.rounding));

    search->active_searching = anim->active;

    text_field_ex(name, "", buf, size, SCALE(12), SCALE(14), draw->get_clr(clr->widgets.text, anim->alpha), false, data->rect, &anim->active, 0, 0, 0, true);
